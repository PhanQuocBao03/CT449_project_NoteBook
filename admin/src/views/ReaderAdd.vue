<template>
	<div class="col-md-12">
		<div class="mx-auto border border-color-white" style="width: 30%;">
			<Form @submit="handleRegister" :validation-schema="registerFormSchema" class="p-5 pb-2">
				<div v-if="!successful">
					<div class="form-group text-left align-self-center">
						<label class="text-white mb-3" for="username">Tên đăng nhập</label>
						<Field name="username" type="text" class="form-control" />
						<ErrorMessage name="username" class="error-feedback" />
					</div>
					<div class="form-group text-left align-self-center">
						<label class="text-white mb-3" for="email">E-mail</label>
						<Field name="email" type="email" class="form-control" />
						<ErrorMessage name="email" class="error-feedback" />
					</div>
					<div class="form-group text-left align-self-center">
						<label class="text-white mb-3" for="password">Mật khẩu</label>
						<Field name="password" type="password" class="form-control" />
						<ErrorMessage name="password" class="error-feedback" />
					</div>
					<div class="form-group text-left align-self-center">
						<label class="text-white mb-3" for="role">Bạn là:</label>
		...
		async handleRegister(user) {
			...
			try {
				...
				setTimeout(() => {
					...
					this.$router.push({ name: 'reader' }); // Redirect to login page
				}, 2000);
			} catch (error) {
				console.log(error);
